...
-- ---------------------------------------------------------------------
WITH steps AS (
  SELECT 1 AS step_no, 'Session start'   AS step_name, COUNTIF(reached_session_start)  AS sessions FROM `YOUR_PROJECT.ga4_growth.fct_sessions`
  UNION ALL
  SELECT 2, 'View item',        COUNTIF(reached_view_item)      FROM `YOUR_PROJECT.ga4_growth.fct_sessions`
  UNION ALL
  SELECT 3, 'Add to cart',      COUNTIF(reached_add_to_cart)    FROM `YOUR_PROJECT.ga4_growth.fct_sessions`
  UNION ALL
  SELECT 4, 'Begin checkout',   COUNTIF(reached_begin_checkout) FROM `YOUR_PROJECT.ga4_growth.fct_sessions`
  UNION ALL
  SELECT 5, 'Add shipping',     COUNTIF(reached_add_shipping)   FROM `YOUR_PROJECT.ga4_growth.fct_sessions`
  UNION ALL
  SELECT 6, 'Add payment',      COUNTIF(reached_add_payment)    FROM `YOUR_PROJECT.ga4_growth.fct_sessions`
  UNION ALL
  SELECT 7, 'Purchase',         COUNTIF(reached_purchase)       FROM `YOUR_PROJECT.ga4_growth.fct_sessions`
)
SELECT
  step_no,
  step_name,
